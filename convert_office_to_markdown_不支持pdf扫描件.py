...
def cleanup_md_and_assets(md_path: str | Path) -> None:
    """删除 Markdown 及其同名 _assets 资源目录（失败仅告警，不抛出）。"""
    ...
    if assets.is_dir():
        try:
            ...
            removed.append(assets.name + "/")
        except Exception as exc:
            print(f"  [WARN] 无法删除图片目录 {assets}：{exc}")
    ...
    if path.is_file():
        try:
            ...
            print(f"  [WARN] 无法删除 Markdown {path}：{exc}")
...
def _pdf_text_from_chars(chars: list[dict]) -> str:
    """根据 PyMuPDF 字符坐标重建文本，利用相邻字符横向间距补英文单词空格。"""
    if not chars:
        return ""

    rows: list[list[dict]] = []
    for ch in sorted(chars, key=lambda c: (float(c.get("bbox", (0, 0, 0, 0))[1]), float(c.get("bbox", (0, 0, 0, 0))[0]))):
        bbox = ch.get("bbox") or (0, 0, 0, 0)
        y0 = float(bbox[1])
        placed = False
        for row in rows:
            row_y = float((row[0].get("bbox") or (0, 0, 0, 0))[1])
            size = float(ch.get("size") or 10)
            if abs(row_y - y0) <= max(2.0, size * 0.35):
                row.append(ch)
                placed = True
                break
        if not placed:
    ...
    for row in rows:
        ...
        prev_char = ""
        for ch in sorted(row, key=lambda c: float((c.get("bbox") or (0, 0, 0, 0))[0])):
            bbox = ch.get("bbox") or (0, 0, 0, 0)
            x0 = float(bbox[0])
            x1 = float(bbox[2])
            char = ch.get("c", "")
            ...
            if prev_x1 is not None:
                gap = x0 - prev_x1
                # PDF 里英文词间空格常表现为明显的字符间距；中文连续字符不强制补空格。
                if (
                    gap > max(1.4, size * 0.20)
...
def _pdf_text_from_block(block: dict) -> str:
    ...
    for line in block.get("lines", []):
        ...
        if chars:
            line_text = _pdf_text_from_chars(chars)
            if line_text:
...
def _pdf_text_in_bbox_from_rawdict(
    rawdict: dict, bbox: tuple[float, float, float, float] | None
) -> str:
    """从页面 rawdict 中按单元格 bbox 取字符并重建文本。"""
    if not bbox:
        return ""
    x0, y0, x1, y1 = bbox
    chars: list[dict] = []
            ...
            for span in line.get("spans", []) or []:
                for ch in span.get("chars", []) or []:
                    ...
                    if x0 <= cx <= x1 and y0 <= cy <= y1:
                        item = dict(ch)
                        item["size"] = span.get("size", 10)
                        chars.append(item)
    return _pdf_text_from_chars(chars)


def _pdf_rebuild_table_data_from_rawdict(
    rawdict: dict,
    data: list[list[str | None]],
    cell_grid: list[list[tuple[float, float, float, float] | None]],
) -> list[list[str | None]]:
    """用 PyMuPDF 字符坐标重建 pdfplumber 表格单元格文本，补足英文词间空格。"""
    rebuilt = [list(row) for row in data]
    for ri, row in enumerate(rebuilt):
        cells = cell_grid[ri] if ri < len(cell_grid) else []
        for ci, cell_bbox in enumerate(cells):
            if cell_bbox is None:
                continue
            text = _pdf_text_in_bbox_from_rawdict(rawdict, cell_bbox)
            if text:
                if ci >= len(row):
...
def _pdf_render_image_bbox(
    page: object,
    bbox: tuple[float, float, float, float],
    *,
    raw_size: tuple[int, int] | None = None,
) -> bytes | None:
    """按图片在 PDF 页面上的显示区域截图，保留页面绘制方向和接近原图的像素密度。"""
    try:
        import fitz  # PyMuPDF
        ...
        if raw_size:
            ...
            raw_long = max(int(raw_w or 0), int(raw_h or 0))
            clip_long = max(float(clip.width), float(clip.height))
            if raw_long > 0 and clip_long > 0:
                scale = max(2.0, raw_long / clip_long)
        pix = page.get_pixmap(
...
def _pdf_collect_page_images(
    ...
    def _extract(xref: int) -> tuple[bytes, str, tuple[int, int] | None] | None:
        try:
        ...
        raw_size = (width, height) if width > 0 and height > 0 else None
        return img_bytes, ext, raw_size
    ...
    for info in infos:
        ...
        if not got:
            continue
        raw_bytes, raw_ext, raw_size = got
        rendered_bytes = _pdf_render_image_bbox(
            page,
            (x0, y0, x1, y1),
            raw_size=raw_size,
        )
        img_bytes = rendered_bytes or raw_bytes
        ext = "png" if rendered_bytes else raw_ext
        out.append(
            {
                "xref": xref,
    ...
    try:
        ...
        try:
            for page_index in range(len(fitz_doc)):
                ...
                rawdict = fitz_page.get_text("rawdict", flags=_text_flags)
                ...
                for t in plumber_page.find_tables() or []:
                    ...
                    try:
                        ...
                        cell_grid = []
                    data = _pdf_rebuild_table_data_from_rawdict(rawdict, data, cell_grid)
